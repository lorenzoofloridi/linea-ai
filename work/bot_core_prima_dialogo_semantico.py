...
def estrai(testo, dati, atteso, storia):
    ...
    for field, pattern in {
        'trattamento': r'(?:togliere|curare|trattare|sistemare|rimuovere)(?: una| la| le)? carie|carie|otturazione|otturazioni|devitalizzazione|estrazione(?: di un dente)?|sbiancamento(?: dentale)?|implantologia|impianti(?: dentali)?|ortodonzia|allineatori(?: trasparenti)?|faccette(?: dentali)?|igiene(?: dentale)?|pulizia(?: dei denti)?|visita(?: odontoiatrica)?|controllo',
        'telefono': r'(?<!\w)\+?\d[\d ()-]{5,}\d(?!\w)',
        'tempistica': r'(?:(?:nel|a|per il) )?(?:breve|lungo|medio)(?: (?:periodo|termine))?|(?:sto )?raccogliendo informazioni(?: per il futuro)?|a breve|(?:per il )?futuro|fra (?:un|due|tre|\d+) mes[ei]|tra (?:un|due|tre|\d+) mes[ei]|(?:questa|prossima) settimana|non subito|a lungo (?:periodo|termine)|più avanti|non ho fretta|appena possibile|il prima possibile|subito|prima possibile|(?:tra|fra) (?:un paio di|qualche|alcuni|una|un|due|tre|\d+) (?:giorni|giorno|settimane|settimana|mesi|mese|anni|anno)|(?:oggi|domani|dopodomani)|(?:il )?prossimo mese',
    }.items():
    ...
    if atteso == 'tempistica' and not out.tempistica:
        if t.strip(' .!?') in ('non so', 'non saprei', 'ancora non so', 'da decidere', 'da concordare', 'non ho ancora deciso', 'più in là', 'tra un po', "tra un po'", 'fra un po'):
            out.tempistica = testo
    name = re.search(r'mi chiamo\s+([^.,;!?\n]+)', testo, re.I)
    if name:
        out.nome = name.group(1).strip()
    elif atteso == 'nome' and re.fullmatch(r"[A-Za-zÀ-ÿ'’ -]{2,80}", testo) and out.intento == 'altro' and not out.tempistica and not out.trattamento and t not in ('no','non so','ciao'):
        out.nome = testo
    ...
    def ricevi(self, testo):
        ...
        for campo, domanda in QUESTIONS.items():
            ...
            if not self.dati.get(campo):
                ...
                if campo == 'tempistica' and prima.get('trattamento') and not result.tempistica:
                    domanda = 'Per completare la richiesta mi manca quando: puoi indicare, per esempio, “subito”, “fra qualche mese” oppure “da decidere”.'
...
def main():
    print('CLINICA DEMO SORRISO — versione 10 settembre, revisione 2 — dimostrazione, non una clinica reale.')
