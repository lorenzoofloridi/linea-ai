...
class EmailService:
 def send(self,cid,event,to,subject,text):
  from . import mail
...
def preview():
 ...
 cards=[]
 for row in rows:
  cards.append('<article><h2>'+html.escape(row['subject'])+'</h2><p>A: '+html.escape(row['recipient'])+' · '+html.escape(row['mode'])+' · '+html.escape(row['created_at'])+'</p><pre style="white-space:pre-wrap">'+html.escape(row['body'])+'</pre><details><summary>HTML del messaggio</summary><pre>'+html.escape(row['html'])+'</pre></details></article>')
