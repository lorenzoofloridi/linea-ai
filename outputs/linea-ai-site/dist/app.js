...
$('#reset-chat').addEventListener('click',()=>{if(busy)return;if(messages.children.length>1&&!confirm((window.lineaPreferences?.t||String)('Iniziare una nuova conversazione? Le richieste già registrate rimangono nell’area dell’azienda.')))return;session=null;sessionPromise=null;consentPending=false;leadSaved=false;if($('#open-contact-picker'))$('#open-contact-picker').disabled=false;messages.replaceChildren();addMessage(greetingText,'bot');$('#feedback').hidden=true;$('#feedback-form').reset();$('#feedback-result').textContent='';$('#feedback-form button').disabled=false;$('#suggestions').hidden=false;$('#chat-note').textContent='Conversazione privata · dati di prova';$('#chat-input').focus()});
