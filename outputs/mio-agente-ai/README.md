...
Il progetto ufficiale online usa `netlify/functions/api.mjs`, PostgreSQL su Supabase (Frankfurt, connessione in `netlify/lib/db.mjs`) e il frontend `outputs/linea-ai-site/dist`. Il ramo operativo è `dev/netlify-backend`; il merge in `main` richiede conferma del proprietario. Registrazione/sessioni sono portate parzialmente; chat, lead e integrazioni non sono ancora operative online. La verifica aggiornata e il contratto delle API sono in [Audit Netlify](../../netlify/AUDIT.md).

AI online: le Functions chiamano Google Gemini direttamente tramite l'adapter `netlify/lib/ai/` (`provider.mjs` + `gemini.mjs`), con chiave `LINEA_GEMINI_API_KEY` presente solo nelle variabili d'ambiente Netlify; Ollama Cloud (`ollama.mjs`, `OLLAMA_API_KEY`/`OLLAMA_MODEL`) è un provider di riserva attivo solo con `LINEA_AI_PROVIDER=ollama`. Chat (`netlify/lib/online-ai.mjs`) e ricerca aziendale (`netlify/lib/company-research.mjs`) usano lo stesso adapter. La ricerca legge fino a 6 pagine pubbliche del sito ufficiale (redirect e DNS verificati verso soli IP pubblici) e, con Gemini, integra la ricerca web; le fonti sono salvate distinguendo sito e web. Prima di ogni chiamata chat il backend riserva un messaggio nella quota mensile dell'azienda (`netlify/lib/ai-quota.mjs`, tabella `ai_usage`, migrazione 0008): Demo 100, Base 3.000, Plus 10.000, Advanced 50.000 messaggi per mese di calendario Europe/Rome; le chiamate fallite non vengono conteggiate. `GET /api/ai-usage` mostra all'azienda autenticata periodo, piano, limite e consumo. La background function di ricerca richiede `LINEA_INTERNAL_SECRET`.

Database online: PostgreSQL su Supabase. Le Functions si collegano con `LINEA_DATABASE_URL` (Transaction pooler, porta 6543) e verificano sempre il certificato TLS con `LINEA_DATABASE_CA`. Le migrazioni sono in `database/migrations` (non più in `netlify/database`, quindi Netlify non le applica durante il deploy) e si applicano dal Mac con `npm run db:status`, `npm run db:migrate` e `npm run db:check`, leggendo `.env.supabase` (ignorato da Git). La tabella `schema_migrations` registra nome e checksum di ogni migrazione applicata; una migrazione già applicata non va modificata. La migrazione 0009 attiva la Row Level Security su tutte le tabelle e revoca i privilegi ai ruoli pubblici di Supabase (`anon`, `authenticated`): il backend accede come proprietario, nessun client esterno può leggere i dati. Ogni nuova tabella deve attivare la RLS nella propria migrazione; `db:check` segnala le tabelle scoperte.
...
## Verifica aziende e amministrazione locale

`saas/verification.py` gestisce `pending`, `under_review`, `verified`, `rejected`, `suspended`. La registrazione parte in attesa; la presentazione/modifica dei dati porta in revisione; il gestore decide approvazione/rifiuto con motivazione. Un rifiuto richiede una nuova revisione prima dell’approvazione. La sospensione revoca l’accesso operativo, le conversazioni attive e i rinnovi mock, mantenendo account e dati. Una successiva riabilitazione è una decisione esplicita del gestore. Le precedenti approvazioni locali sono migrate, senza verifiche automatiche presso registri ufficiali.

La pagina Account mostra lo stato; GET `/api/company-verification` restituisce esclusivamente stato e cronologia della propria azienda. Nessuna API aziendale consente auto-approvazione o sospensione di un’altra azienda. Il pannello amministrativo è per ora uno strumento **locale da terminale**, non una pagina web con ruoli amministrativi.

Dalla cartella `outputs/linea-ai-site`, con l’interprete della `.venv`:

```sh
../mio-agente-ai/.venv/bin/python -m saas.verification list
../mio-agente-ai/.venv/bin/python -m saas.verification review ID_AZIENDA --status under_review --reason "Controllo dei documenti"
../mio-agente-ai/.venv/bin/python -m saas.verification review ID_AZIENDA --status verified --reason "Verifica locale completata"
../mio-agente-ai/.venv/bin/python -m saas.verification history ID_AZIENDA
```

L’audit conserva azienda, stato precedente/nuovo, identità dell’utente del computer, motivazione e data. Il computer del gestore è il confine di fiducia: un futuro pannello remoto richiederà ruoli amministrativi autenticati. `subscriptions.VerificationProvider` rimane il punto di estensione per registri esterni; nessun registro è stato collegato.

## Autorizzazioni dei piani

`saas/plan_entitlements.py` è la matrice controllata dal server per dashboard, conversazioni, lead, esportazione, prenotazioni, CRM, voce, operatore e WhatsApp. La disponibilità effettiva è l’intersezione di piano valido, stato aziendale e permessi dell’agente; una modifica dal browser non concede funzionalità escluse. Il backend protegge sia API sia azioni dell’agente.

Poiché i servizi commerciali dei singoli piani non sono ancora stati definiti, la matrice iniziale mantiene le funzionalità esistenti per tutti i piani: **non sono state inventate differenze commerciali**. È configurabile centralmente; i test verificano che disabilitare esportazione/prenotazioni nella matrice le blocchi davvero. I permessi aziendali per azioni sensibili restano disattivati finché autorizzati.

`plan_subscriptions` conserva ora anche `expired`, scadenza della tolleranza e cambio piano programmato. La migrazione locale ricrea il vincolo degli stati preservando le righe, poi aggiunge le colonne mancanti; viene verificata anche su schema precedente e su avvii ripetuti. `plan_commands` deduplica le riattivazioni. `company_verification` e `company_verification_audit` conservano workflow e revisioni. Prima di modifiche allo schema creare un backup privato; questo non sostituisce ancora un framework generale di migrazioni di produzione.
...
- **Demo:** 7 giorni dall’attivazione, una volta per email normalizzata. La scadenza è nel database, non nel browser. Alla scadenza il riquadro scompare; i dati restano conservati. Il backend protegge dashboard, API private e apertura della chat aziendale di prova. Anche gli account preesistenti devono attivare una prova o un piano. Il backend ricontrolla l’accesso anche sulle conversazioni già iniziate: scadenza o sospensione fermano ulteriori messaggi. Non sono ancora previste quote quantitative di messaggi/token.
- **Piani a pagamento simulati:** Base €299/mese o €3.049,80/anno (equivalente €254,15/mese); Plus €599/mese o €6.109,80/anno (€509,15/mese); Advanced €999/mese o €10.189,80/anno (€849,15/mese). Tutte le tariffe annuali risparmiano il 15% sul totale di 12 mesi. Servizi inclusi ancora da definire. La prova dura 14 giorni; richiede dati aziendali, approvazione manuale del gestore, metodo simulato ricorrente e conferma esplicita. Una sola prova commerciale per azienda, indipendentemente dal piano scelto. Plus e Advanced hanno gli stessi passaggi di verifica, metodo simulato, prova di 14 giorni e disdetta del Base. Upgrade e downgrade sono programmabili dalla gestione del piano e diventano effettivi al prossimo rinnovo riuscito, senza conguaglio. È possibile modificare nuovamente il cambio programmato; non è prevista una seconda prova gratuita.
- **Dati aziendali:** ragione sociale, identificativo fiscale, sito, email e telefono aziendali, referente e ruolo, sede, città, CAP e Paese. Il formato viene controllato, ma questo non dimostra l’identità o la validità fiscale. Modificare i dati richiede una nuova revisione. Il workflow aziendale mantiene allineati il profilo commerciale e l’abilitazione delle installazioni, conservando i campi storici per compatibilità.
...
- **Rinnovo e disdetta:** un worker locale controlla ogni 30 secondi; il controllo avviene anche quando si legge lo stato. Alla scadenza registra un pagamento simulato e attiva il periodo successivo, salvo disdetta. La disdetta dalla pagina di gestione mantiene accesso fino alla scadenza. Quando il server è spento non esegue attività: alla ripartenza elabora lo stato scaduto, senza ricostruire tutti i periodi trascorsi. Un fallimento passa a `past_due` con 7 giorni di tolleranza (`GRACE_DAYS` nel backend). Alla fine diventa `expired` e l’accesso cessa. Il pulsante di riattivazione permette di riprovare con un nuovo pagamento mock: il successo riapre un periodo, il fallimento non concede accesso né prolunga la tolleranza. Una chiave idempotente evita addebiti duplicati per lo stesso comando. Ripristinare il rinnovo prima della scadenza non genera pagamenti; riattivare dopo la scadenza richiede pagamento e verifica aziendale. Le aziende sospese/rifiutate non vengono addebitate dal worker.
