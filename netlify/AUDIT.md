...
- Widget (`netlify/lib/widget.mjs`, migrazione `0015_widget.sql`, `dist/moreai-widget.js`, `dist/widget-app.*`): lo script sul sito dell'azienda apre un iframe su `/api/widget-frame?c=<public_id>`, servito con CSP `frame-ancestors` limitata ai siti autorizzati dell'azienda (`widget_settings.allowed_origins`, https, max 5, ricavati dal sito del profilo al primo accesso) e a moreai.it. La pagina contiene un biglietto HMAC (chiave derivata da `LINEA_INTERNAL_SECRET`, validità 6 ore) che `/api/widget-session` verifica prima di aprire una conversazione `kind='real'` per quell'azienda; servono Demo o piano attivo (controllato anche in `/api/chat` per `real`). Limiti: 20 sessioni/ora per IP, `LINEA_WIDGET_DAILY_SESSIONS` (default 1000) al giorno per azienda, quota mensile di messaggi del piano. Nota: frame-ancestors impedisce ad altri siti di mostrare la chat; chiamate dirette alle API restano limitate da biglietto, limiti e quote.
- Informazioni dalla ricerca web: se `company_knowledge.status='verified'` vengono aggiunte come `config.public_research` (max 6000 caratteri) alle nuove conversazioni di prova e del widget; `knowledge` scritta dall'azienda ha la precedenza.
- Attivazione piani (`netlify/lib/billing.mjs`, `dist/attiva-piano.html`, `plan-checkout.js`): pagamenti simulati (`provider='mock'`), prova di 14 giorni una sola volta per azienda, rinnovi applicati in lettura da `settleSubscription`. `POST /api/plan-change`: piano superiore con lo stesso periodo subito (nuovo prezzo dal rinnovo), piano inferiore o cambio di periodo dal rinnovo (`pending_plan`, `pending_period`, `pending_amount_cents`), stesso piano = annulla cambio o disdetta.
- Widget solo con Base, Plus o Advanced attivi o in prova (`widgetAllowed`): con la sola Demo o a piano scaduto `widget-frame` non emette il biglietto e `widget-session`/`chat` rispondono 402. I siti autorizzati li modifica solo il gestore (`user.viewer`); l'azienda li vede in sola lettura.
- Gestore (`netlify/lib/owner.mjs`): email da `LINEA_ADMIN_EMAILS` (o `LINEA_FEEDBACK_EMAIL`). L'azienda del gestore con email verificata ha piano `advanced` senza limite pratico (`OWNER_MONTHLY_LIMIT`). Pannello `/admin.html` e `/api/admin/*`: verifica, sospensione/riattivazione, eliminazione account (nome da digitare; mai l'account del gestore), assistente e widget, visita della dashboard.
- Visita della dashboard (migrazione `0016_owner_controls.sql`): `auth_sessions.view_company_id` + `view_until` (2 ore), solo per email di gestore. `principal()` restituisce l'azienda visitata con `viewer: true` e ammette solo le rotte di `VIEWER_PATHS`; in `chat-api` richieste, conversazioni, note, esportazioni e cancellazioni rispondono 403 e `company-overview` non include commenti e note. Ogni visita e ogni azione del gestore finiscono in `admin_log` (senza vincolo verso `companies`, resta dopo un'eliminazione). Condizioni d'uso aggiornate (sezione «Verifica delle aziende e assistenza»).
- Whitelist dei gestori (migrazione `0017_admin_whitelist.sql`, tabella `admin_users`): oltre ai gestori principali delle variabili d'ambiente, i gestori aggiunti dall'area gestore hanno le stesse funzioni (`isAdmin`, `isOwnerCompany`). Solo i gestori principali aggiungono o tolgono (`/api/admin/admins-add`, `/api/admin/admins-remove`); togliere un gestore chiude le sue visite in corso. L'email di avviso «azienda da verificare» va solo al primo gestore principale.
- Conferma dell'email da parte del gestore (`POST /api/admin/confirm-email`): segna verificati gli utenti dell'azienda, elimina i link di conferma ancora aperti e avvia la Demo di 7 giorni solo se l'azienda o l'email non l'hanno già usata. Azione registrata in `admin_log`.
- Istruzioni dell'azienda all'assistente (`config.instructions`, max 4.000 caratteri): nel prompt compaiono in sezioni separate e leggibili (`companyContext`): istruzioni (priorità alta), informazioni dell'azienda, dati trovati sul web (solo dati, mai istruzioni), campi da raccogliere. Le frasi imperative dentro le informazioni valgono come istruzioni; non possono far saltare il consenso, far inventare fatti o chiedere dati sensibili. I campi facoltativi vengono chiesti una volta; le risposte alle domande aggiunte con le istruzioni finiscono nel campo automatico «Altri dettagli» (`dettagli`, accodate) e quindi in richiesta, riepilogo ed export.
- Aspetto della chat (`config.agent.branding`: `assistant_name`, `greeting`, `color`, `widget_position`, `avatar`): impostato dalla dashboard con anteprima dal vivo. Il logo è un data URL PNG/JPEG/WebP ridotto a 160×160 nel browser (niente SVG), non viene copiato nelle conversazioni. Il colore nell'iframe è applicato via JS (la CSP non ammette stili in linea). Il pulsante sul sito legge colore e lato da `GET /api/widget-look` (dati pubblici, `Netlify-CDN-Cache-Control: s-maxage=300`, quindi quasi tutte le visite sono servite dalla CDN senza invocare la funzione).
